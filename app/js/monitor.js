...
const cpuOverload = 40;
const alertFrequence = 5;
...
setInterval(() => {
  ...
  cpu.usage().then((info) => {
    ...
    // Check overload
    if (info >= cpuOverload && runNotify(alertFrequence)) {
      notifyUser({
        title: 'CPU overload',
  ...
  // Uptime
  document.getElementById('sys-uptime').innerText = secondsToDmhs(os.uptime());
}, 2000);
...
function secondsToDmhs(seconds) {
  seconds = +seconds;
